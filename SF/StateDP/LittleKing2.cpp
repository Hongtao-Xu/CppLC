...
LL f[2][K][M];       // f[i][j][s]所有只摆了前i行，已经摆了j个国王并且第i行摆放状态是s的所有方案集合
...
int main() {
    ...
    for(int i=1;i<=n+1;++i)//枚举每一行
        for(int j=0;j<=m;++j)//枚举国王数量
            ...
            {
                //要先清空,因为第一维一直在循环,转移用的 += ，不清空会用到前前阶段的状态
                f[i&1][j][state[a]]=0;//第一维：一直循环0，1，0，1...
                for(int b:head[a]){
    ...
    cout << f[(n + 1)&1][m][0] << endl;//我们假设摆到n + 1行，并且另这一行状态为0,那么即得到我们想要的答案， 
}
